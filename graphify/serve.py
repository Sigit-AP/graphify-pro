...
def _query_graph_text(
    ...
    *,
    ...
    # Algoritma Growth: query hits are recall events that strengthen long-term
    # memory. Best-effort, no-op when the store is absent — never affects query
    # results, only the sidecar memory state.
    if graph_path:
        try:
            from graphify.growth import record_query_recalls
            _db = str(Path(graph_path).with_suffix(".growth.db"))
            record_query_recalls(_db, start_nodes)
        except Exception:
            pass
