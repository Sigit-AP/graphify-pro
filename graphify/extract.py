...
from .lsp_resolution import resolve_python_instance_member_calls
...
# Python instance member-call resolution via jedi static inference (50x Fase 3):
# resolves ``obj.method()`` — which the shared call pass skips on bare-name
# collision — to the true definition by receiver type. OPTIONAL: jedi is imported
# lazily inside the resolver; when absent it is a no-op and graphify stays
# zero-dependency. Lives in graphify.lsp_resolution.
register_language_resolver(
    LanguageResolver(
        "python_instance_member_calls",
        frozenset({".py"}),
        resolve_python_instance_member_calls,
    )
)
